class AlbumAggregatorJob
  ...
  def populate_aggregates
    ...
    albums = DetailSummary.select(:album_id).where(date: @date).distinct.limit(500)
    ...
    albums.each do |album|
      ...
      Resque.enqueue(AggregateAlbumByDate, @date, album.album_id)
    end
    ...
    while ( num_remaining = $redis.scard(@working_queue_key) ) > 0
      ...
      if repeat_count >= 5
        ...
        $redis.smembers(@working_queue_key).each { |a| Resque.enqueue(AggregateAlbumByDate, @date, a) }
        repeat_count = 0
      end

      previous_remaining = num_remaining

      Rails.logger.info "Waiting for aggregation to complete (#{@num_albums - num_remaining}/#{num_albums} remaining)"
      sleep(60)
    end
